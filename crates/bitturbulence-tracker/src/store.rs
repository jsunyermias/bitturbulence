...
impl PeerStore {
    ...
    fn load_from_db(&self) -> Result<()> {
        let db = self.db.lock().map_err(|_| TrackerError::LockPoisoned)?;
        ...
        let mut inner = self.inner.lock().map_err(|_| TrackerError::LockPoisoned)?;
        ...
        for row in rows {
            ...
            if ih.len() != 32 || pid.len() != 32 { continue; }
            let mut info_hash = [0u8; 32];
            let mut peer_id   = [0u8; 32];
            info_hash.copy_from_slice(&ih);
            peer_id.copy_from_slice(&pid);

            let record = PeerRecord { info_hash, peer_id, addr, uploaded: up,
                downloaded: down, left, last_seen, completed };
    ...
    pub fn announce(&self, req: &AnnounceRequest) -> Result<Vec<PeerRecord>> {
        ...
        let result: Vec<PeerRecord> = peers.values()
    ...
    pub fn scrape(&self, info_hash: &[u8; 32]) -> (u32, u32, u32) {
        let inner = self.inner.lock().expect("tracker store mutex poisoned");
    ...
    pub fn flush(&self) -> Result<()> {
        let inner = self.inner.lock().map_err(|_| TrackerError::LockPoisoned)?;
        let db    = self.db.lock().map_err(|_| TrackerError::LockPoisoned)?;
...
mod tests {
    ...
    fn stopped_removes_peer() {
        ...
        store.announce(&make_req(&"aa".repeat(32), AnnounceEvent::Started, 100)).unwrap();
        store.announce(&make_req(&"aa".repeat(32), AnnounceEvent::Stopped, 100)).unwrap();
    ...
    fn peers_exclude_self() {
        ...
        store.announce(&make_req(&pid, AnnounceEvent::Started, 100)).unwrap();
        store.announce(&make_req(&"bb".repeat(32), AnnounceEvent::Started, 100)).unwrap();

        let peers = store.announce(&make_req(&pid, AnnounceEvent::Update, 100)).unwrap();
        assert!(peers.iter().all(|p| p.peer_id != parse_hex32(&pid).unwrap()));
    }
    ...
    fn flush_and_reload() {
...

        {
            ...
            store.announce(&make_req(&"aa".repeat(32), AnnounceEvent::Started, 500)).unwrap();
            store.flush().unwrap();
        }
